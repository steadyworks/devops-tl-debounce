...
from worker.pool import WorkerPoolSupervisor
from worker.process.base import BaseWorkerProcess
...
def test_worker_starts_and_receives_heartbeat(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for cls in supervisor.processes:
        ...
        assert not supervisor._is_worker_dead(p, conn)
...
def test_worker_restart_on_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    supervisor._is_worker_dead = lambda p, c: True  # type: ignore
